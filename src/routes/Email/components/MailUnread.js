...
import { MailUnreadMessageLi, MailUnreadButton } from '../modules/classnamevars'

const getMessages = (messages, handleClick, handleDelete) => {
  if (messages.length > 0) {
    return messages.map((message, i) => {
      return (
        <li className={`${MailUnreadMessageLi} ${getMarginLeftOdd(i)}`} key={`message-unread-${i}`}>
          <div className='containerSubject mb4 pb4 bb b--moon-gray cf'>
            ...
            <div className='date tl'>{message.date}</div>
            <div className='delete blue pointer tl' onClick={() => handleDelete(message.id)}>Delete</div>
          </div>
