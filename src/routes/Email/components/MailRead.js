    ...
    return messages.map((message, i) => {
      return (
        <li className='message bg-white mb1 pv3 ph3 ba b--moon-gray br2 cf' key={`message-read-${i}`}>
          <div className='cell containerMessage relative-l fr-l w-90-l ph3-l pt0-l'>
            ...
            <div className='containerTimestamp absolute-l top-0-l right-0-l tl tr-l'>
              <div className='cell timestamp moon-gray'>{message.date}</div>
              <div className='cell delete blue pointer' onClick={() => handleDelete(message.id)}>Delete</div>
            </div>
          </div>
          <div className='cell containerBtn'>
            <button className='br2 ph3 pv2 mt3 mt0-l bg-orange white ba b--orange ws-normal pointer tl' onClick={() => handleClick(message.id)}>Mark Unread</button>
          </div>
        </li>
      )
    })
